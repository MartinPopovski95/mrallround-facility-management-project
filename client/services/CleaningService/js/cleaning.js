...
(() => {
    ...
    const API_URL = API_CONFIG.getApiUrl('/api/cleaning-service');
    ...
    let globalBasePrice = 0;
    let quoteRequestEmail = 'info@mrallround.ch'; 

    function renderOfferings(offerings) {
        if (!dom.offeringsContainer || !offerings || offerings.length === 0) {
            return;
        }
    ...
    /**
     * Render service packages from CMS data
     * Expected data structure from Strapi:
     * [
     *   {
     ...
     *     basePrice: 150,
     *     isDefault: false,
     *     description: "Comprehensive end-of-tenancy cleaning" (optional)
     *   },
     *   ...
     * ]
     */
    function renderServicePackages(packages) {
        if (!dom.packageSelect || !packages || packages.length === 0) {
            return;
        }

        servicePackagesData = packages;

        dom.packageSelect.innerHTML = '';
        ...
        sortedPackages.forEach((pkg, index) => {
            ...
            option.value = pkg.value || String(index + 1);

            const priceText = pkg.basePrice != null
                ? ` - CHF ${pkg.basePrice}`
                : '';
            option.textContent = `${pkg.name || 'Unknown Package'}${priceText}`;

            option.setAttribute('data-cms-managed', 'true');
            ...
            dom.packageSelect.appendChild(option);
        });

        if (window.serviceCalculator) {
            const event = new Event('change', { bubbles: true });
            dom.packageSelect.dispatchEvent(event);
        }
    }

    /**
     * Get base price for a service package by value
     * @param {string} packageValue - The package value identifier
     * @returns {number|null} Base price or null if not found
     */
    function getPackageBasePrice(packageValue) {
        const pkg = servicePackagesData.find(p => p.value === packageValue);
        return pkg ? (pkg.basePrice || 0) : null;
    }

    /**
     * Get global base price for the service
     * @returns {number} Global base price
     */
    function getGlobalBasePrice() {
        return globalBasePrice || 0;
    }

    /**
     * Get area pricing tiers from CMS for a specific package
     * @param {string} packageValue - The package value identifier
     * @returns {Array} Array of pricing tiers for the package
     */
    function getAreaPricingTiers(packageValue) {
        if (!packageValue) return [];

        const pkg = servicePackagesData.find(p => p.value === packageValue);
        if (pkg && pkg.areaPricingTiers) {
            return pkg.areaPricingTiers;
        }

        return [];
    }

    window.getPackageBasePrice = getPackageBasePrice;
    window.getGlobalBasePrice = getGlobalBasePrice;
    window.getAreaPricingTiers = getAreaPricingTiers;
    window.getQuoteRequestEmail = () => quoteRequestEmail;
    ...
    async function loadCleaningService(locale = "en") {
        ...
        try {
            ...
            queueMicrotask(() => {
                ...
                globalBasePrice = data.globalBasePrice || 0;

                if (data.quoteRequestEmail) {
                    quoteRequestEmail = data.quoteRequestEmail;
                }

                if (data.servicePackages && data.servicePackages.length > 0) {
                    renderServicePackages(data.servicePackages);
                }
    ...
    window.loadCleaningService = loadCleaningService;
})();
