...
(() => {
    ...
    const API_URL = API_CONFIG.getApiUrl('/api/property-maintenance-service');
    ...
    function initOfferingsCarousel(offeringCards) {
        ...
        for (let i = 0; i < totalOfferings; i++) clonedOfferings.push(offeringCards[i].cloneNode(true));
        for (let i = 0; i < totalOfferings; i++) clonedOfferings.push(offeringCards[i]);
        for (let i = 0; i < totalOfferings; i++) clonedOfferings.push(offeringCards[i].cloneNode(true));
        ...
        controls.innerHTML = `
            <button class="carousel-btn carousel-prev" aria-label="Previous offering"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg></button>
            <button class="carousel-btn carousel-next" aria-label="Next offering"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg></button>`;
    ...
    async function loadPropertyMaintenanceService(locale = "en") {
        setLoading(true);
        ...
        try {
            const data = await fetchJSON(`${API_URL}?locale=${encodeURIComponent(locale)}`);
    ...
    function initServiceFormModal() {
        ...
        if (serviceFromInput) serviceFromInput.setAttribute('min', today);
        if (serviceToInput) serviceToInput.setAttribute('min', today);
        ...
        form.addEventListener('submit', async function(event) {
            ...
            const formData = {
                ...
                objectSize: getValue('objectSize'),
                rooms: getValue('rooms'),
                kitchen: getValue('kitchen'),
                bathrooms: getValue('bathrooms'),
                windows: getValue('windows'),
                windowSize: getValue('windowSize'),
                blinds: getValue('blinds'),
                terrace: getValue('terrace'),
                serviceFrom: getValue('serviceFrom'),
                serviceTo: getValue('serviceTo'),
            ...
            const createEndpoint = API_CONFIG.getApiUrl('/api/service-requests');
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        initLanguageSwitch(loadPropertyMaintenanceService);
    });

    window.loadPropertyMaintenanceService = loadPropertyMaintenanceService;
})();